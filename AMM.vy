...
def tradeTokens(sell_token: address, sell_quantity: uint256):
	...
	if sell_token == self.tokenA.address:
		...
		total_A: uint256 = sell_quantity + self.tokenAQty
		total_B: uint256 = self.invariant / total_A
		Bsend: uint256 = self.tokenBQty - total_B
		self.tokenB.transfer(msg.sender, Bsend)
		self.tokenAQty = total_A
		self.tokenBQty = total_B
	else:  # sell_token is tokenB
		self.tokenB.transferFrom(msg.sender, self, sell_quantity)
		total_B: uint256 = self.tokenBQty + sell_quantity
		total_A: uint256 = self.invariant / total_B
		Asend: uint256 = self.tokenAQty - total_A
		self.tokenA.transfer(msg.sender, Asend)
		self.tokenAQty = total_A
		self.tokenBQty = total_B
